...
// and false when
//   - a content hash is provided that mismatches with the current version
// For safety, cargo-husky does nothing on case2 also.
fn hook_already_exists(hook: &Path, content_hash: Option<u64>) -> bool {
    ...
    if let Some(hash) = content_hash {
        if !ver_line.contains("(content hash: ") {
            // No content hash has been added.
            return false;
        }
        let hash_marker = format!("(content hash: {:016x})", hash);
        if !ver_line.contains(&hash_marker) {
            // The content changed.
            return false;
        }
    }
...
fn install_hook(hook: &str) -> Result<()> {
    ...
    if !hook_already_exists(&hook_path, None) {
...
fn install_user_hook(src: &Path, dst: &Path) -> Result<()> {
    ...
    // Compute a deterministic hash of the hook source content for change detection
    let content_hash = {
        use std::collections::hash_map::DefaultHasher;
        use std::hash::{Hash, Hasher};
        // Use a fixed seed hasher for determinism across runs
        let mut hasher = DefaultHasher::default();
        // Hash each line separately with a stable representation
        for line in &lines {
    ...
    if hook_already_exists(&dst_file_path, Some(content_hash)) {
        return Ok(());
    }

    // Insert cargo-husky package version information and content hash as comment
    ...
    lines.insert(
        2,
        format!(
            "# This hook was set by cargo-husky v{}: {} (content hash: {:016x})",
            env!("CARGO_PKG_VERSION"),
            env!("CARGO_PKG_HOMEPAGE"),
            content_hash,
        ),
...
fn install_user_hooks() -> Result<()> {
    ...
    // Tell cargo to rerun cargo-husky if a user hook changes.
